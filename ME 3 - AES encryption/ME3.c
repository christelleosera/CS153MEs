...
int main() {
	char option[8], fname[25],  test[999], key[17], input[17], hex_val[3];
	FILE *fp, *fo;
	int i = 0, j =0, current = 0, temp = 0, x, y, ron, n;
	int key_block[4][4], input_block[4][4], input_P[4][4], input_S[4][4], tempo[4], round_no = 1, exp_key[4][44], t[4], prev[4], holder[4], prev_4[4];
	int s_box[256] = {			
			...
			39, 178, 117, 9, 131, 44, 26, 27, 110, 90, 160, 82, 59, 214, 179, 
			41, 227, 47, 132, 83, 209, 0, 237, 32, 252, 177, 91, 106, 203, 190, 
			57, 74, 76, 88, 207, 208, 239, 170, 251, 67, 77, 51, 133, 69, 249, 
			...
			213, 78, 169, 108, 86, 244, 234, 101, 122, 174, 8, 186, 120, 37, 46, 
	...
	int inv_s_box[256] = 
	{
			...
			203, 84, 123, 148, 50, 166, 194, 35, 61, 238, 76, 149, 11, 66, 250, 
			195, 78, 8, 46, 161, 102, 40, 217, 36, 178, 118, 91, 162, 73, 109, 
			...
			175, 189, 3, 1, 19, 138, 107, 58, 145, 17, 65, 79, 103, 220, 234, 
			151, 242, 207, 206, 240, 180, 230, 115, 150, 172, 116, 34, 231, 173, 53, 
			133, 226, 249, 55, 232, 28, 117, 223, 110, 71, 241, 26, 113, 29, 41, 
			197, 137, 111, 183, 98, 14, 170, 24, 190, 27, 252, 86, 62, 75, 198, 
			210, 121, 32, 154, 219, 192, 254, 120, 205, 90, 244, 31, 221, 168, 51, 
			136, 7, 199, 49, 177, 18, 16, 89, 39, 128, 236, 95, 96, 81, 127, 
			169, 25, 181, 74, 13, 45, 229, 122, 159, 147, 201, 156, 239, 160, 224, 
			59, 77, 174, 42, 245, 176, 200, 235, 187, 60, 131, 83, 153, 97, 23, 
	...
	gets(option);
	} while(strcmp(option,"encrypt") != 0 && strcmp(option, "decrypt") != 0);
	...
	printf("Enter 128-bit key (16 ASCII characters): ");
	gets(key);
	...
	if(strcmp(option,"encrypt") == 0)sprintf(fname, "%s_e.txt", fname);
	else sprintf(fname, "%s_d.txt", fname);
	fo = fopen(fname, "w");
	

	
	//strcpy(test, "NoIdon'tgotmoneyButIgotthetimehoneyIfyou'llwait for me to turn this all around I'm in debt to you always As we make the same mistakes Just know jealousy's no longer in the way Let's learn to put the past behind us Let love push through the door and find us Just keep your heart wide open So I can fix what's broken Stay around maybe I can be a better me");
	
	
	//strcpy(key, "*Gv0^as!d8L0$V+@");
	//printf("8985682C8FAB338EFEAE54B53A01539C");
	//convert key into block
	for(i=0; i<16; i++) key_block[i%4][i/4] = key[i];
	
	for(i=0; i<4; i++){
		for(j=0; j<4; j++){
			printf("%d ", key_block[i][j]);
		}printf("\n");
	}
	
	
	//for(i=0; i<16; i++) { input[i] = test[current]; current++; }
	
	input[16] = '\0';
	printf("%s\n", input);
	for(i=0;i<16;i++) printf("%X ", input[i]);
	printf("\n");
	...
	for(i=0; i<4; i++){
		for(j=0; j<4; j++){
			exp_key[i][j] = key_block[i][j];
			if(j == 3) prev[i] = key_block[i][j];
			if(j == 1) prev_4[i] = key_block[i][j];
		}
	}
	ron = 1;
	temp = i;
	printf("KEYYYY\n");
	while(i < 44){
		for(j=0; j<4; j++){ t[j] = exp_key[j][i-1]; printf("%d ", t[j]); }printf("\n");
		
		for(j=0; j<4; j++)tempo[j] = t[j];
		for(j=0; j<4; j++)holder[j] = t[(j+1) % 4];
		
		for(j=0; j<4; j++){
			hex_val[1] = '\0'; hex_val[0] = '\0';
			sprintf(hex_val,"%X", holder[j] % 256);
			if(hex_val[1] == '\0') { hex_val[1] = hex_val[0]; hex_val[0] = '0'; };
			printf("%s ", hex_val);
			
			if(hex_val[0] >= 'A' && hex_val[0] <= 'F') x = (hex_val[0] - 55);
			else if (hex_val[0] >= '0' && hex_val[0] <= '9') x = (hex_val[0] - 48);
			
			if(hex_val[1] >= 'A' && hex_val[1] <= 'F') y = (hex_val[1] - 55);
			else if (hex_val[1] >= '0' && hex_val[1] <= '9') y = (hex_val[1] - 48);
			
			holder[j] = s_box[(x*16) + (y)];
			sprintf(hex_val,"%X", holder[i] % 256);
			if(hex_val[1] == '\0') { hex_val[1] = hex_val[0]; hex_val[0] = '0'; };
		}
		holder[0] = holder[0] ^ (int)pow(2, (ron - 1));
		
		
		ron++;
		printf("\n");
		for(j=0; j<4; j++){t[j] = holder[j];printf("%d ", t[j]); }printf("\n");printf("%d\n", i);
		for(j=0; j<4; j++){t[j] = t[j] ^ exp_key[j][i-4];printf("%d ", t[j]); }printf("\n");
		//i++;
		
		for(j=0; j<4; j++)exp_key[j][i] = t[j] % 256;
		n=i;
		for(i; i<(n+4); i++){
			for(j=0; j<4; j++){t[j] = t[j] ^ exp_key[j][i-4];printf("%d ", t[j]); }printf("\n");
			for(j=0; j<4; j++)exp_key[j][i] = t[j] % 256;
	...
	round_no = 1;
	while(fread(input, 1, 16, fp) > 0){
		input[16] = '\0';
		printf("%s\n", input);
		//convert input into block
		for(i=0; i<16; i++) input_block[i%4][i/4] = input[i];
	...
	if(strcmp(option, "encrypt") == 0){
			...
			for(i=0; i<4; i++){
				for(j=0; j<4; j++){
					input_P[i][j] = (input_block[i][j] ^ key_block[i][j]);
				}
			}
			...
			for(i=0; i<4; i++){
				for(j=0; j<4; j++){
					hex_val[1] = '\0'; hex_val[0] = '\0';
					sprintf(hex_val,"%X", input_P[i][j] % 256);
					if(hex_val[1] == '\0') { hex_val[1] = hex_val[0]; hex_val[0] = '0'; };
					printf("%s ", hex_val);
			...
			for(i=0; i<4; i++){
				for(j=0; j<4; j++){
				input_P[i][j] = input_S[i][(j+i)%4];
				}
				for(j=0; j<4; j++) input_S[i][j] = input_P[i][j];
			}
			...
			if(round_no < 10){
				//mix columns
				for(j=0; j<4; j++){
				tempo[0] = (table_2[input_S[0][j]] ^ table_3[input_S[1][j]] ^ (input_S[2][j]) ^ (input_S[3][j])) % 256;
				tempo[1] = ((input_S[0][j]) ^ table_2[input_S[1][j]] ^ table_3[input_S[2][j]] ^ (input_S[3][j])) % 256;
				tempo[2] = ((input_S[0][j]) ^ (input_S[1][j]) ^ table_2[input_S[2][j]] ^ table_3[input_S[3][j]]) % 256;
				tempo[3] = (table_3[input_S[0][j]] ^ (input_S[1][j]) ^ (input_S[2][j]) ^ table_2[input_S[3][j]]) % 256;
				for(i=0; i<4; i++) input_P[i][j] = tempo[i];

				}

				for(i=0; i<4; i++){
					for(j=0; j<4; j++){
						input_S[i][j] = input_P[i][j];
					}
				}
			...
			for(i=0; i<4; i++){
				for(j=0; j<4; j++){
					input_S[i][j] = (input_S[i][j] ^ exp_key[i][j+(round_no*4)]) % 256;
				}
			}
			...
			for(i=0; i<4; i++){
				for(j=0; j<4; j++){
					input[(i*4)+j] = input_S[j][i];
				}
			}
			
			fwrite(input, 1, 16, fo);
	...
	//add round key
			for(i=0; i<4; i++){
				for(j=0; j<4; j++){
					input_S[i][j] = input_S[i][j] ^ key_block[i][j];
				}
			}
			...
			while(round_no <= 10){
				...
				for(i=0; i<4; i++){
					for(j=0; j<4; j++){
						input_S[i][j] = (input_S[i][j] ^ exp_key[i][j+((11 - round_no)*4)]) % 256;
					}
				}
				...
				//mix columns
					...
					tempo[0] = (table_14[input_S[0][j]] ^ table_11[input_S[1][j]] ^ table_13[input_S[2][j]] ^ table_9[input_S[3][j]]) % 256;
					tempo[1] = (table_9[input_S[0][j]] ^ table_14[input_S[1][j]] ^ table_11[input_S[2][j]] ^ table_13[input_S[3][j]]) % 256;
					tempo[2] = (table_13[input_S[0][j]] ^ table_9[input_S[1][j]] ^ table_14[input_S[2][j]] ^ table_11[input_S[3][j]]) % 256;
					tempo[3] = (table_11[input_S[0][j]] ^ table_13[input_S[1][j]] ^ table_9[input_S[2][j]] ^ table_14[input_S[3][j]]) % 256;
					
					for(i=0; i<4; i++) input_S[i][j] = tempo[i];
					}
